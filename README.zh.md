...
**渐进式加载**
skill 通过 `Skill` 工具按需加载，不会预先污染上下文。
...
创建 `~/.claude/CLAUDE.md`，使用渐进式导入（保持上下文加载量在 60 行以内）：
...
**前置依赖：** 先安装 superpowers for Cursor，参考 [superpowers 安装说明](https://github.com/obra/superpowers)。

```bash
git clone https://github.com/fast-harness/fast-harness ~/.cursor/plugins/fast-harness
```

在 Cursor 设置中添加（`~/.cursor/settings.json` 或项目级）：
...
# 其他平台 — 拉取仓库
cd ~/.codex/fast-harness && git pull   # 根据你的平台调整路径
```
...
fast-harness 通过 Claude Code 中的 **skill 系统**工作（其他平台有对应机制）。会话开始时，`using-superpowers` skill 被注入，它指导 agent 在响应任何任务之前先调用相关 skill。三个 fast-harness skill 随后按需加载——不是一次性全部加载。

这意味着：
- 不需要时零上下文开销
- 恰好在相关时刻加载完整指导
- skill 与 superpowers 和官方插件组合，而不是替换它们

`harness-engineering` skill 尤其实现了**反压（back-pressure）**——抵抗长对话中 AI 自然产生的漂移、循环、或虚报完成倾向的机制。这是核心洞察：AI 会话需要和软件系统一样的工程纪律。
...
请保持 skill 简洁——token 效率很重要（见 harness-engineering 原则：工具超过 15 个开始降级，超过 30 个明显退化）。
